...
def llm_server_kvbm(request, runtime_services_dynamic_ports):
    ...
    This fixture always wires the KVBM connector
    (``kvbm.vllm.connector``) with the NIXL UCX backend enabled.
    """
    import os
    import time
    ...
    # The harness always wires the KVBM KvbmConnector.
    ...
    env.update(
        {
            ...
            # KvbmConnector uses NATS_SERVER and ETCD_ENDPOINTS from the env.
        }
    )
    ...
    # Build vLLM command. always wire the KVBM KvbmConnector.
    kv_transfer_config = '{"kv_connector":"KvbmConnector","kv_connector_module_path":"kvbm.vllm.connector","kv_role":"kv_both"}'
    command = [
        "vllm",
        "serve",
        "--block-size",
    ...
    # KVBM requires the NIXL UCX backend to be enabled.
